        ...
        <div ref={reveal()} className="mt-10 sm:mt-12 lg:mt-16 rounded-[2rem] bg-gray-900 px-6 py-8 sm:px-10 sm:py-10 shadow-xl overflow-hidden relative">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 sm:gap-10 relative z-10">
            <div className="text-center sm:text-left">
              <h2 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">{content['doctors_cta_heading']}</h2>
              <p className="text-gray-400 text-sm sm:text-base mt-2 max-w-md mx-auto sm:mx-0 leading-relaxed">{content['doctors_cta_subtext']}</p>
            </div>
            <div className="flex flex-wrap justify-center sm:justify-end gap-3 sm:gap-4 shrink-0 w-full sm:w-auto mt-2 sm:mt-0">
              <a href={`tel:+${phone}`} className="w-full sm:w-auto inline-flex justify-center items-center gap-2 rounded-xl bg-brand-red px-6 py-3.5 text-sm font-bold text-white hover:bg-brand-red-dark transition-colors shadow">
                <PhoneCall className="h-4 sm:h-5 w-4 sm:w-5" /> {content['doctors_cta_btn_call'] || 'Call Now'}
              </a>
              <a href={`https://wa.me/${whatsapp}`} target="_blank" rel="noopener noreferrer"
                ...
                <MessageCircle className="h-4 sm:h-5 w-4 sm:w-5" /> {content['doctors_cta_btn_whatsapp'] || 'WhatsApp'}
              </a>
              <Link to="/contact" className="w-full sm:w-auto inline-flex justify-center items-center gap-2 rounded-xl border border-white/20 px-6 py-3.5 text-sm font-semibold text-white hover:bg-white/10 backdrop-blur-md transition-colors">

  ...
  return (
    <footer className="bg-gray-950 pt-8 pb-4 sm:pt-14 sm:pb-6 text-gray-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap justify-between gap-x-6 gap-y-7 sm:gap-y-10 sm:gap-x-10 mb-6 sm:mb-10 text-left">
          ...
          <div className={`w-full sm:w-auto flex-1 min-w-50 flex flex-col items-center sm:items-start space-y-3 sm:space-y-4 ${visClass(logoDesktop, logoMobile)}`}>
            ...
            <p className="text-xs leading-relaxed text-gray-400 max-w-xs text-center sm:text-left">{tagline}</p>
          ...
          {/* Quick Links */}
          <div className={`w-[45%] sm:w-auto flex-1 min-w-[140px] flex flex-col items-start ${visClass(qlDesktop, qlMobile)}`}>
            <h3 className="text-white font-bold text-sm mb-3 sm:mb-4 flex items-center justify-start gap-1.5 w-full sm:w-auto">
              <HeartPulse className="h-4 w-4 text-brand-green hidden sm:block" />
              {qlHeading}
            </h3>
            <ul className="space-y-2 flex flex-col items-start w-full sm:w-auto text-left">
              {FOOTER_HREFS.map((href, i) => {
                ...
                return (
                  <li key={href}>
                    <Link
                      to={href}
                      className="text-[13px] sm:text-xs hover:text-brand-green transition-colors flex items-center gap-1.5"
                    >
          ...
          {/* Get in Touch */}
          <div className={`w-[45%] sm:w-auto flex-1 min-w-[140px] flex flex-col items-start ${visClass(contactDesktop, contactMobile)}`}>
            <h3 className="text-white font-bold text-sm mb-3 sm:mb-4 text-left">{contactHeading}</h3>
            <ul className="space-y-2.5 sm:space-y-3.5 flex flex-col items-start w-full sm:w-auto">
              <li className="flex flex-row items-start gap-2.5">
                ...
                <div className="text-left">
                  ...
                  <a href={`tel:+${phone}`} className="text-[13px] sm:text-xs font-medium text-white hover:text-brand-green transition-colors">
                    {phoneDisplay}
                  </a>
                </div>
              </li>
              <li className="flex flex-row items-start gap-2.5">
                ...
                <div className="text-left">
                  ...
                  <a href={`mailto:${email}`} className="text-[13px] sm:text-xs font-medium text-white hover:text-brand-green transition-colors break-all">
                    {email}
                  </a>
                </div>
              </li>
              <li className="flex flex-row items-start gap-2.5">
                ...
                <div className="text-left">
                  ...
                  <span className="text-[13px] sm:text-xs leading-relaxed whitespace-pre-line inline-block max-w-45">{addressShort}</span>
                </div>
              </li>
            </ul>
          </div>
          ...
          <div className={`w-full sm:w-auto flex-1 sm:flex-2 min-w-0 sm:min-w-60 flex flex-col items-start ${visClass(hoursDesktop, hoursMobile)}`}>
            <h3 className="text-white font-bold text-sm mb-3 sm:mb-4 text-left">{hoursHeading}</h3>
            <div className="bg-gray-900/60 rounded-xl p-3 sm:p-4 border border-gray-800 inline-block w-auto max-w-sm sm:w-full">
              <ul className="space-y-2 text-left">
                <li className="text-[13px] sm:text-xs text-gray-300 whitespace-nowrap">{hoursWeekday}</li>
                <li className="h-px w-full bg-gray-800" />
                <li className="text-[13px] sm:text-xs text-gray-300 whitespace-nowrap">{hoursSunday}</li>
              </ul>
            </div>
          </div>
        </div>
